...
class OneDimRealNVP(nn.Module):
    ...
    def __init__(self, dim, hidden_dim = 32, base_network=FCNN):
        super().__init__()
        self.dim = dim
        self.t1 = base_network(dim, dim, hidden_dim)
        self.s1 = base_network(dim, dim, hidden_dim)
        self.t2 = base_network(dim, dim, hidden_dim)
        self.s2 = base_network(dim, dim, hidden_dim)

    def forward(self, x):
        
        t1_transformed = self.t1(x)
        s1_transformed = self.s1(x)
        x = t1_transformed + x * torch.exp(s1_transformed)
        t2_transformed = self.t2(x)
        s2_transformed = self.s2(x)
        z = t2_transformed + x * torch.exp(s2_transformed)
    
        t1_transformed_grads = autograd.grad(t1_transformed, x, grad_outputs=torch.ones_like(t1_transformed), retain_graph=True, create_graph=True, allow_unused=True)[0]
        s1_transformed_grads = autograd.grad(s1_transformed, x, grad_outputs=torch.ones_like(s1_transformed), retain_graph=True, create_graph=True, allow_unused=True)[0]
        t2_transformed_grads = autograd.grad(t2_transformed, x, grad_outputs=torch.ones_like(t2_transformed), retain_graph=True, create_graph=True, allow_unused=True)[0]
        s2_transformed_grads = autograd.grad(s2_transformed, x, grad_outputs=torch.ones_like(s2_transformed), retain_graph=True, create_graph=True, allow_unused=True)[0]
        print(s1_transformed_grads)
        log_det = torch.sum(torch.log(torch.abs(torch.exp(s1_transformed)*(1 + x*s1_transformed_grads) + t1_transformed_grads)+1e-4), dim=1) + \
                  torch.sum(torch.log(torch.abs(torch.exp(s2_transformed)*(1 + x*s2_transformed_grads) + t2_transformed_grads)+1e-4), dim=1)
        return z, log_det
...
class MAF(nn.Module):
    ...
    """
    def __init__(self, dim, hidden_dim = 64, base_network=FCNN):
        super().__init__()
        self.dim = dim
...
class NSF_AR(nn.Module):
    ...
    [Durkan et al. 2019]
    """
    def __init__(self, dim, K = 5, B = 3, hidden_dim = 32, base_network = FCNN):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
